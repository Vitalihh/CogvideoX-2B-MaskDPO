...
class CogVideoXMaskedVideoToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def load_mask_from_folder(
        ...
        mask_files = sorted(
            [
                os.path.join(mask_folder_path, n)
                for n in os.listdir(mask_folder_path)
        ...
        mask_3d = np.stack(mask_frames, axis=0)  # [F, H, W]
        mask_tensor = torch.from_numpy(mask_3d).to(dtype=dtype)
        mask_tensor = mask_tensor.unsqueeze(0).unsqueeze(0)  # [1, 1, F, H, W]
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            if mask is None:
                raise ValueError("Mask is not provide.")
            mask = mask.to(device=video.device, dtype=video.dtype)
           
            import torch.nn.functional as F
            # 不改变前两维度
            init_mask = F.interpolate(
                mask,
                size=init_latents[0].shape[-3:], 
                mode='trilinear',
                align_corners=False
            )
            init_mask = [init_mask.repeat(1, init_latents[0].shape[1], 1, 1, 1)]
            # import pdb;pdb.set_trace()

            init_latents = torch.cat(init_latents, dim=0).to(dtype).permute(0, 2, 1, 3, 4)  # [B, F, C, H, W]
            init_mask = torch.cat(init_mask, dim=0).to(dtype).permute(0, 2, 1, 3, 4)  # [B, F, C, H, W]
            x0_latents = self.vae_scaling_factor_image * init_latents

            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            xt_latents = self.scheduler.add_noise(x0_latents, noise, timestep)

            # mask_latents = init_mask * latents + (1 - init_mask) * init_latents

            # ####
            # video = self.decode_latents(mask_latents)
        ...
        xt_latents = xt_latents * self.scheduler.init_noise_sigma
        return xt_latents, x0_latents, init_mask, noise
    ...
    def __call__(
        ...
        if latents is None:
            ...
            if mask_folder_path is None:
                raise ValueError("Please provide `mask_folder_path`.")
            if mask_folder_path is not None:
                mask = self.load_mask_from_folder(
                    mask_folder_path,
        ...
        latents, x0_latents, mask, noise = self.prepare_latents(
            video,
            mask,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                xt_latents = self.scheduler.add_noise(x0_latents, noise, t)
                latents = mask * latents + (1 - mask) * xt_latents
        ...
        return CogVideoXPipelineOutput(frames=video), latents
