...
from.Bezier_3dmask import generate_3d_video_masks
...
class CogVideoXVideoToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            if mask is None:
                mask = generate_3d_video_masks(video.size(2), height, width) # B, C, F, H, W]
                mask = mask.to(device=video.device, dtype=video.dtype)
                
            # init_mask = [retrieve_latents(self.vae.encode(vid.unsqueeze(0)), generator) for vid in mask]
            import torch.nn.functional as F
            # init_mask = [mask.resize(init_latents[0].size())]
    ...
    def __call__(
        ...
        mask: Optional[torch.Tensor] = None,
        latent_timestep: Optional[torch.Tensor] = None,
        ...
        latents, x0_latents, mask, prepared_noise = self.prepare_latents(
            video,
            mask,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                xt_latents = self.scheduler.add_noise(x0_latents, prepared_noise, t)
